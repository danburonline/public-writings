# Public Writings

This repository hosts all public writings by Daniel Burger: essays, papers, patents, and books. Everything is written in LaTeX using a unified template system in `_templates/`. See [`_templates/README.md`](_templates/README.md) for template documentation.

Contributions welcome — these are living documents, continuously updated post-publication.

## Repository Structure

```txt
Writings/
├── 01_patents/     # Technical inventions
├── 02_papers/      # Academic papers and preprints
├── 03_books/       # Extended works with chapter structure
├── 04_essays/      # Long-form scientific/philosophical writing
└── _templates/     # Shared LaTeX templates
```

## Patents

| Title | Date | Status |
|-------|------|--------|

## Papers

| Title                        | Venue     | Status      |
| ---------------------------- | --------- | ----------- |
| Brain-in-Vatism: [Title TBC] | HCII 2026 | In progress |

## Books

| Title | Date | Status |
|-------|------|--------|

## Essays

| Title                                             | Date         | Status      |
| ------------------------------------------------- | ------------ | ----------- |
| Mind Uploading: A Candidate for the Great Filter  | —            | In progress |
| Death is an Engineering Challenge                 | May 13, 2025 | Published   |
| Lab-Grown Human Brain Embodied in a Virtual World | Oct 18, 2024 | Published   |
