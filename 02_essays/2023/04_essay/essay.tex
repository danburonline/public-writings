...
\begin{sloppypar} % For better line breaks
  ...
  In response to these limitations, the utilisation of alternative models is increasingly being recommended by the scientific community. In vitro cell cultures, for instance, offer a means of studying biological processes in neurons without the use of live animals, while in silico models, which simulate neuronal structures on computers, have demonstrated success in recent research by (CITE). Another promising avenue is the use of brain organoids, such as the neurospheres developed by Swiss startup FinalSpark, which can provide valuable insights into bidirectional neural interfacing without the need for in vivo animal models and can persist for several months \citep{finalspark_artificial_2022}.
