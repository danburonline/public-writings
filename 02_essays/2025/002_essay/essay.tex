...
\begin{sloppypar}
  ...
  \begin{figure}[ht!]
    ...
    \caption[The four-dimensional process-world-line representing conscious continuity]{\textbf{The four-dimensional process-world-line representing conscious continuity.} \textbf{Left:} A conscious substrate (defined as the physically localised system, e.g., brain, whose organisation supports the continuous physical-dynamical activity (\textbf{1}) underpinning consciousness) evolves through space (\textbf{2}) and time (\textbf{3}), generating an unbroken process-world-line (\textbf{4}). \textbf{Middle:} Cessation (i.e., death (\textbf{5})) occurs when substrate degradation or failure leads to the irreversible termination of these essential processes, halting the world line and signifying the loss of conscious continuity. \textbf{Right:} Synconetics aims to prevent this irreversible termination by actively sustaining the process-world-line (\textbf{6}). This involves engineering interventions such as countering entropy within the substrate (e.g., via energy/matter exchange, error correction) or facilitating the non-destructive migration and, therefore, substrate transition (\textbf{7}) of the core physical-dynamical processes to an alternative—potentially more resilient—substrate, thereby ensuring unbroken processual continuity.}
